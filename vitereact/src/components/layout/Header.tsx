  ...
  return (
    <header className={`py-5 md:py-6 px-6 sm:px-8 md:px-12 lg:px-16 flex justify-between items-center gradient-luxury text-ivory relative z-50 sticky top-0 transition-all duration-300 ${scrolled ? 'shadow-luxury backdrop-blur-md bg-opacity-95' : ''}`}>
      ...
      <nav className="hidden lg:flex flex-1 justify-center">
        <div className="flex gap-8 xl:gap-10 font-body font-medium text-base tracking-wide">
          ...
          <Link to="/ai-services" className="hover:text-gold transition-premium relative group flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-gold/60 animate-pulse"></span>
            AI Services
            <span className="absolute -bottom-1 left-0 w-0 h-[1px] bg-gold transition-all duration-300 group-hover:w-full"></span>
          </Link>
      ...
      {/* Mobile Expanded Menu - Dropdown for AI Services, Pricing and Contact */}
      {expandedMobileNav && (
        <div className="lg:hidden absolute top-full left-0 right-0 elevated-card-dark backdrop-blur-md shadow-luxury z-40 animate-fadeIn border-t border-gold/10">
          <nav className="flex flex-col p-6 gap-2">
            <Link
              ...
              className="font-body font-medium text-base tracking-wide text-gold hover:text-ivory transition-colors py-3 px-4 rounded-lg hover:bg-gold/5 flex items-center gap-2"
            >
